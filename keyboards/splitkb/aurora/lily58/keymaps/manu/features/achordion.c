...
bool achordion_opposite_hands(const keyrecord_t* tap_hold_record, const keyrecord_t* other_record) {
    bool th_on_left    = on_left_hand(tap_hold_record->event.key);
    bool other_on_left = on_left_hand(other_record->event.key);
    dprintln(th_on_left ? "Tap hold record on: left" : "Tap hold record on: right");
    dprintln(other_on_left ? "Other record on: left" : "Other record on: right");
    return th_on_left != other_on_left;
}
